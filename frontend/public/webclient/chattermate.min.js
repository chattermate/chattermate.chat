(()=>{var B=(k,M,e)=>new Promise((b,y)=>{var C=p=>{try{c(e.next(p))}catch(u){y(u)}},L=p=>{try{c(e.throw(p))}catch(u){y(u)}},c=p=>p.done?b(p.value):Promise.resolve(p.value).then(C,L);c((e=e.apply(k,M)).next())});window.chattermateId;window.ChatterMate;(function(){function k(t){return/^#[0-9A-F]{6}$/i.test(t)}function M(){return"http://localhost:8000/api/v1"}let e={baseUrl:M(),containerId:"chattermate-container",buttonId:"chattermate-button",chatBubbleColor:"#f34611",loadingContainerId:"chattermate-loading",tokenKey:"ctid",containerBottom:100,containerRight:20,containerWidth:400,chatInitiationMessages:[],initiationMessageId:"chattermate-initiation",initiationShownKey:"ctim_shown"};function b(){let t=document.createElement("style");t.id="chattermate-styles",t.textContent=`
      #${e.buttonId} {
        position: fixed;
        bottom: 20px;
        right: 20px;
    ...
    `;let i=document.getElementById("chattermate-styles");i&&i.remove(),document.head.appendChild(t)}function y(){return window.chattermateToken||localStorage.getItem(e.tokenKey)}function C(t){t&&localStorage.setItem(e.tokenKey,t)}function L(){localStorage.removeItem(e.tokenKey)}function c(){return window.innerWidth<=768}function p(t){return t?t.replace(/\uFFFD/g,"").replace(/[��]/g,"").replace(/[\x00-\x1F\x7F-\x9F]/g,"").trim():""}function u(){if(!e.chatInitiationMessages||e.chatInitiationMessages.length===0)return null;let t=Math.floor(Math.random()*e.chatInitiationMessages.length),i=e.chatInitiationMessages[t];return i?p(i):null}function H(){return c()||!e.chatInitiationMessages||e.chatInitiationMessages.length===0?!1:!sessionStorage.getItem(e.initiationShownKey)}function E(){sessionStorage.setItem(e.initiationShownKey,"true")}function F(){let t=u();if(!t)return null;let i=document.createElement("div");return i.id=e.initiationMessageId,i.innerHTML=`
