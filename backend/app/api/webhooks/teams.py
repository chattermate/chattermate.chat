...
async def teams_webhook(
    ...
    """Receive a Microsoft Teams (Bot Framework) Activity. Validates the Bot
    Connector JWT, acks immediately, and processes the message in the
    background."""
    account = ChannelAccountRepository(db).get_by_id(account_id)
    if account is None or account.channel_type != ChannelType.TEAMS.value:
        raise HTTPException(status_code=404, detail="Unknown account")

    adapter = get_adapter(ChannelType.TEAMS.value)
    ...
    for inbound in adapter.parse_inbound(payload):
        if is_duplicate_message(ChannelType.TEAMS.value, f"{account.id}:{inbound.external_message_id}"):
            logger.info(f"Skipping duplicate Teams activity {inbound.external_message_id}")
            continue
